...
- `backend/`：后端服务，包含小程序接口、运营后台接口和相关测试。
- `console/`：运营后台前端。
...
- 后端：`Node.js + Express + JWT + MySQL`，部署主链路为 CloudBase 云托管。
...
- `docs/console/邻动体适能运营后台/` 属于文档型原型目录，非当前主业务入口；正式实现优先以 `miniprogram/`、`backend/`、`console/` 为准。
