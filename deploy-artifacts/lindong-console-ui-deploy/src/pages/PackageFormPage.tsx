...
export function PackageFormPage({ mode }: { mode: PackagePageMode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const normalizedConfig = normalizeGroupPriceConfig(data.group_price_config || [])
  ...
  const setGroupPriceConfig = (nextConfig: PackageDetail['group_price_config']) => {
    const normalizedConfig = normalizeGroupPriceConfig(nextConfig)
    setForm(current => ({
      ...current,
      group_price_config: normalizedConfig,
      supported_people: deriveSupportedPeople(normalizedConfig)
  ...
  const addGroupPriceRow = () => {
    setForm(current => ({
      ...current,
      group_price_config: [...current.group_price_config, { target_count: 0, price_fen: 0 }]
    }))
  }
      ...
      {!loading ? (
        <form className="stack" onSubmit={handleSubmit}>
          <div className="form-grid">
            ...
            <label>
              <span>总价参考价（分）<RequiredMark /></span>
              <input
                type="number"
                min="1"
            ...
            {form.group_price_config.length ? (
              <div className="group-price-stack">
                {form.group_price_config.map((item, index) => (
                  <div key={`group-price-${index}`} className="group-price-row">
                    <label>
          ...
          <section className="panel subtle-panel stack">
            ...
            <div className="detail-grid">
              <div className="detail-card">
                ...
                <p>支持团型：{supportedPeopleText}</p>
                <p>参考价：¥{(Number(form.total_price_fen || 0) / 100).toFixed(2)}</p>
