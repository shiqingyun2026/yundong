...
### 12.1 2026-05-09 拼团时长测试回切

2026-05-09 做过一次“小程序拼团时长临时改为 5 分钟”的本地/联调测试，当前已回切完成：

1. 开团截止逻辑已恢复为按课包配置的 `deadline_hours` 计算
2. 当前固定业务口径仍为“开团后 48 小时截止”
3. 若后续再次做超短时长联调，请务必在测试完成后同步回切代码与测试断言，避免误带入正式环境
