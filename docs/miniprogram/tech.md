...
| V1.3 | 2026-04-27 | 技术负责人 | 同步当前真实生产口径：小程序后端主链路切换为 CloudBase 云托管请求头 + MySQL 数据层；登录升级为静默基础身份 + 手机号强身份 + 同手机号跨微信号归并 |

## 1. 技术架构概述

### 1.1 当前整体架构

当前系统采用“微信原生小程序前端 + Node.js/Express 后端 + CloudBase 云托管身份头 + MySQL 数据层”的开发架构：

* **前端**：微信小程序原生框架，负责页面渲染、用户交互、调用微信原生能力（登录、定位、分享、图片预览）。
* **后端**：Node.js + Express，提供 RESTful API，处理登录、课程、拼团、订单、支付、账号归并等业务逻辑。
* **身份来源**：云托管请求自动携带 `x-wx-openid / x-wx-appid / x-wx-unionid`，后端据此识别微信身份。
* **数据库**：MySQL，后端通过 repository 层访问。
...
| 登录封装 | `utils/auth.js` | 统一处理 `wx.login` 静默登录、手机号快速验证、会话持久化 |
...
| 数据库 | MySQL | 通过 repository 层访问 |
| 鉴权 | JWT Bearer Token | `jsonwebtoken` 签发，`Authorization: Bearer <token>` |
| 微信身份 | CloudBase 云托管请求头 | 使用 `x-wx-openid / x-wx-appid / x-wx-unionid` |
| 环境变量 | dotenv | 管理 `PORT / MYSQL_* / JWT_SECRET / TRUST_CLOUDBASE_MINIPROGRAM_IDENTITY` |

### 1.4 当前核心业务流程

1. 小程序启动后执行 `wx.login` 静默登录，请求后端 `/api/auth/login` 建立基础身份与 token。
2. 用户可在未完成手机号强身份的情况下浏览首页和课程详情。
3. 云托管请求会自动携带微信身份头，后端据此识别当前微信 `openid`。
4. 用户触发“立即开团 / 去参团 / 我的拼团”等核心动作时，如未完成手机号强身份，则拉起登录弹层。
5. 登录弹层通过“手机号快速验证”能力获取手机号验证 `code`，请求后端 `/api/auth/phone`。
6. 后端用手机号完成当前账号绑定；若该手机号已绑定在其他微信号主账户下，则执行账号归并，并返回新的 token。
7. 首页获取定位并请求课程列表；若网络不可用则回退默认坐标和 mock 数据。
8. 课程详情页请求课程详情和当前进行中的拼团信息。
9. 用户从详情页点击“立即开团/去参团”后，进入支付确认链路并创建订单。
10. 支付确认页请求课程信息和可选的拼团信息，再发起支付。
11. 支付成功后进入支付成功态的拼团详情页，查看实时拼团状态。
12. “我的拼团”列表页展示当前主账户参与的拼团，并支持状态筛选、倒计时、下拉刷新、上拉加载更多。
